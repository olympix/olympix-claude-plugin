...
> **⛔ Each agent must finish ALL its work within its single turn — a dispatched subagent is NOT re-invoked.** When a subagent's turn ends, it is done; no harness wake-up resumes it. Each agent must drive its entire flow to completion **in-turn**: poll/wait in a **blocking loop** (use the `Monitor` tool or a foreground loop between polls), and for BugPocer drive the full interactive flow (scope → validation → **security questions** → submit) to `validation_submitted`, then poll the scan, then retrieve+save. Return **only after** results are retrieved and saved. An agent must **never** background a poll/`sleep` (or pause mid-flow) and end its turn expecting to be resumed — the wake-up never comes, so the backend session keeps going but nothing is retrieved (the exact gap that drops mutation/unit results and skips BugPocer security questions). `run_in_background: true` applies to *launching* the agent so the user can keep chatting — **not** to the agent's own polling. The `sleep 900` heartbeat below is the **orchestrator's** (main-loop) mechanism only; subagents do not get it. State this explicitly in each agent's dispatch prompt.
