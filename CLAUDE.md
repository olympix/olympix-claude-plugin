...
- BugPocer directed scans are opt-in: only when the user asks to focus the scan, add `--directed` with `--domains <ids>` and/or `--directions-file <path>` (both require `--directed`) and answer the `directed_scope` event with `confirm_directed`. Never make it a mandatory question. See the bug-pocer skill, Step 1.6.
