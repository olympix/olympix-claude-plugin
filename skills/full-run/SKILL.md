...
> **Keep launching all the agents concurrently with `run_in_background: true` — that backgrounding is the whole point: it lets the user keep chatting with you while the tools run. Do NOT run the tools synchronously in the main loop.** The rule below is only about what each agent does *inside its own run*, not about whether to background them.
>
> **⛔ Once running, each agent must finish ALL its work within that single agent run — a subagent is NOT re-invoked.** When the agent's turn ends it is done; no wake-up resumes it. So *inside the agent*: wait in a **blocking loop** (use the `Monitor` tool to block until the session reaches `Completed`/`Failed`, or `InitialScanCompleted` for BugPocer), drive the whole flow (BugPocer: scope → validation → **security questions** → submit → scan), then retrieve + save, then return. The agent must **never** background its *own* poll/`sleep` and end its turn expecting to be resumed — that wake-up never comes, so the backend session keeps running but nothing is retrieved (the gap that dropped mutation/unit results and skipped BugPocer security questions). Backgrounding happens at the orchestrator level (launching the agent); the agent itself runs straight through. The `sleep 900` heartbeat below is the **orchestrator's** mechanism only. State this explicitly in each agent's dispatch prompt.
